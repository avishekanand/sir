...
def load_trec_covid():
    ...
    for i, q in enumerate(dataset.queries_iter()):
        ...
        qrels = [
            ...
            if qr.query_id == q.query_id and qr.relevance > 0
        ]
...
def load_bright(domain="biology"):
    ...
    queries_ds = load_dataset(
        "xlangai/BRIGHT", "examples", split=domain, streaming=True
    )
    queries = []
    gold_ids_all = set()
    for i, q in enumerate(queries_ds):
        if i >= SAMPLES_PER_DATASET:
            break
        queries.append(
            {"id": f"q{i}", "text": q["query"], "gold_ids": set(q["gold_ids"])}
        )
    ...
    for i, d in enumerate(corpus_ds):
        if i >= 5000 and d["id"] not in gold_ids_all:
            continue
        if i >= 10000:
            break  # Hard limit
