...
def load_bright_data(domain: str):
    ...
    for i, q in enumerate(ds):
        if i >= QUERIES_PER_DOMAIN:
            break
        # Ensure standard keys for evaluate function
        queries.append({"query": q["query"], "gold_ids": q["gold_ids"]})

    # Load corpus
    corpus_ds = load_dataset(
        "xlangai/BRIGHT", "documents", split=domain, streaming=True
    )
...
def evaluate(controller, queries):
    ...
    for q in queries:
        ...
        results.append(
            {"found": found, "docs_reranked": docs_reranked, "latency": elapsed}
        )
...
def run_benchmark():
    ...
    for domain in DOMAINS:
        ...
        docs = [
            Document(page_content=c["content"], metadata={"id": c["id"]})
            for c in corpus
        ]
    ...
    print(
        final_df[
            ["domain", "method", "accuracy", "avg_docs_reranked", "avg_latency"]
